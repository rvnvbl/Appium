...
public class TC6_0_CheckOut_Android_NoPWalletBalance_PayInStore extends AndroidSetup {
   ...
   public void PWalletMethodTest() throws Exception {
      ...
      try {
         ...
         AndroidElement TopUpButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "dialog_top_up_balance_option_top_up")));
   ...
   @Test(priority = 12)
   public void PWalletTopUpTest() throws Exception {
      try {
         // Clicking the Top Up button
         AndroidElement TopUpButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "dialog_top_up_balance_option_top_up")));
         ...
         markTestStatus("failed", "FAILED: Top Up button error / not found.", driver);
         errorcounter++;
      }

      try {
         // Checking if successfully redirected to page
         AndroidElement TopUpTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "title")));

         if ((TopUpTitle.getText()).equals("Cash In")) {
            System.out.println("Successfully redirected to Top Up page");
         } else {
            markTestStatus("failed", "FAILED: Did not redirect to Top Up page", driver);
            errorcounter++;
         }
      ...
      try {
         // Going back to payment method page and checking if successfully redirected to
         // page
         driver.navigate().back();

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);

         AndroidElement PaymentMethodTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("title")));

         if ((PaymentMethodTitle.getText()).equals("Payment Method")) {
            System.out.println("Successfully redirected to Payment Method page");
         } else {
            markTestStatus("failed", "FAILED: Did not redirect to Payment Method page", driver);
            errorcounter++;
   ...
   @Test(priority = 13)
   public void PWalletModalChangePaymentTest() throws Exception {
      try {
         // Clicking the Puregold Wallet payment option
         AndroidElement PWalletOption = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy
               .AndroidUIAutomator("new UiScrollable(new UiSelector()" + ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
   ...
   public void ApplyVoucherTest() throws Exception {
      ...
      try {
         ...
         MobileElement SubtotalElement = (MobileElement) driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()"
   ...
   public void CartCleanUpTest() throws Exception {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Checkout > Pay in Store test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Checkout > Pay in Store Module Successful", driver);
      }
   }
