...
public class TC26_1_CheckOut_Android_PwalletDiscount_Value extends AndroidSetup {
   ...
   public void TapCartButtonTest() throws Exception {
      ...
      try {
         ...
         // AndroidElement SearchedQuantityAdd = (AndroidElement) new WebDriverWait(driver,
   ...
   public void PayInStoreMethodTest() throws Exception {
      ...
      try {
         AndroidElement productTotalValue = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("checkout_bottom_total")));
         productValue = productTotalValue.getText();
         dblProductValue = Double.parseDouble(extractValue(productValue));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Checkout bottom not found", driver);
         errorcounter++;
      }
   ...
   private void retestDiscountModal() {
      // Checking the pwallet discount button
      try {
         AndroidElement pwalletDiscountbutton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("checkout_pwallet_discount_switch")));
      ...
      try {
         AndroidElement pwalletDiscountInfoIcon = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("iv_info")));
         pwalletDiscountInfoIcon.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pwallet Discount icon not found", driver);
         errorcounter++;
      }

      // Discout Pwallet Title
      try {
         AndroidElement availablePwalletBalance = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(
               MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector()" + ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
                     + "new UiSelector().text(\"P-Wallet as Discount\"));")));

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Available PWallet title (Info Page) not found", driver);
         errorcounter++;
      }

      // Discout Pwallet Spiel
      try {
         AndroidElement availablePwalletBalance = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(
               MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector()" + ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
                     + "new UiSelector().text(\"Entered amount will be deducted from your P-Wallet balance.\"));")));

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: PWallet text (Info Page) not found", driver);
         errorcounter++;
      }

      // Checking the close button
      try {
         AndroidElement pwalletDiscountInfoClose = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("btnClose")));
         pwalletDiscountInfoClose.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pwallet Discount icon not found", driver);
         errorcounter++;
      }
   ...
   @Test(priority = 19)
   private void priceAndPwalletSame() {
      // Checking the pwallet discount button
      double dblPwalletDiscount = 0;
      double dblPwalletBalance = 0;
      try {
         AndroidElement pwalletDiscountbutton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("checkout_pwallet_discount_switch")));
         pwalletDiscountbutton.click();
      ...
      // Getting the discount balance available
      try {
         AndroidElement pwalletDiscountValue = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("pwallet_discount")));

         dblPwalletDiscount = Double.parseDouble(pwalletDiscountValue.getText());

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pwallet Discount value not found", driver);
         errorcounter++;
      }

      // Test if the balance is greater han equal pwallet discount value
      try {
         AndroidElement pwalletBalance = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("pwallet_balance")));

         dblPwalletBalance = Double.parseDouble(extractPesoSign(pwalletBalance.getText()));
      ...
      // Checker if the balance is can pay the item amount
      System.out.println("Product Value: " + dblProductValue);
      System.out.println("Wallet Balance: " + dblPwalletBalance);
      System.out.println("Discount Value: " + dblPwalletDiscount);

      if (dblProductValue <= dblPwalletBalance) {
         Assert.assertTrue(dblPwalletDiscount == dblProductValue);
         System.out.println("The product value is lower or equal the pwallet balance");
         markTestStatus("passed", "PASSED: The product value is lower or equal the pwallet balance", driver);
   ...
   @Test(priority = 20)
   private void addQuantityTest() throws InterruptedException {
      // Checking if redirect to Cart page is successful
      try {
      ...
      try {
         AndroidElement addQuantityButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("cart_quantity_add")));

         addQuantityButton.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Add quantity not found", driver);
         errorcounter++;
      }
   ...
   private void priceGreaterDiscountTest() {
      // Checking the pwallet discount button
      double dblPwalletDiscount = 0;
      double dblProductValue = 0;
      double dblPwalletBalance = 0;
      double currentdblRemainProductBalnce = 0;
      double expecteddblRemainProductBalnce = 0;
      ...
      try {
         ...
         markTestStatus("failed", "FAILED: Checkout total not found", driver);
         errorcounter++;
      }
      ...
      // Checker if the balance is can pay the item amount
      if (dblProductValue > dblPwalletBalance) {
         Assert.assertTrue(dblPwalletDiscount == dblPwalletBalance);
         System.out.println("The product value is higher the pwallet balance");
         markTestStatus("passed", "PASSED: The product value is higher the pwallet balance", driver);
         expecteddblRemainProductBalnce = dblPwalletBalance - dblPwalletDiscount;
      }
   ...
   @Test(priority = 28)
   private void errorCounterTest() {
      if (errorcounter > 0) {
         markTestStatus("failed",
               "an error an occurred on Checkout > Pwallet Discount (Less Than / Greater Than Product Price) Error, Please see logs for details",
               driver);
      } else {
         markTestStatus("passed", "Checkout > Pwallet Discount (Less Than / Greater Than Product Price) Passed", driver);
      }
   ...
   public static String extractPesoSign(String input) {
      // Assuming the currency symbol "₱" is always at the beginning
      return input.substring(1); // Extracts from index 1 (skipping the currency symbol)
   }
