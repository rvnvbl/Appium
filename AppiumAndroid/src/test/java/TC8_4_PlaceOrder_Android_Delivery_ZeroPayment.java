...
public class TC8_4_PlaceOrder_Android_Delivery_ZeroPayment extends AndroidSetup {
	...
	public void TapAddAddressTest() throws Exception {
		...
		try {
			MobileElement DeliveryOptionButton = (MobileElement) driver.findElementByAndroidUIAutomator(
					"new UiScrollable(new UiSelector()" + ".scrollable(true)).scrollIntoView"
							+ "(new UiSelector().resourceId(\"com.grocery.puregold:id/home_delivery_layout_image\"));");
	...
	public void StoreHomepageAddToCartTest() throws Exception {
		...
		try {
			MobileElement GlobeProductHeader = (MobileElement) driver
		...
		try {
			MobileElement GlobeSimItem = (MobileElement) driver
					.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()"
		...
		// confirmation
		try {
			AndroidElement ItemDescriptionCheck = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("item_description_title")));
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Item description element not found, redirect to specific product failed.",
					driver);
		}
		...
		// to cart button
		try {
			AndroidElement MinusQuantityButton = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("item_product_quantity_minus")));

			AndroidElement QuantityField = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("item_product_quantity")));

			AndroidElement AddQuantityButton = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("item_product_quantity_add")));
	...
	public void OrderConfirmTest() throws Exception {
		...
		if (errorcounter > 0) {
			markTestStatus("failed",
					"an error an occurred on Place Order > Delivery Zero method Module test, Please see logs for details",
					driver);
		} else {
			markTestStatus("passed", "Place Order > Delivery Zero method Module Successful", driver);
		}
	}
