...
public class TC25_3_BillsPay_Android_NoEcCashCategory extends AndroidSetup {
   ...
   public void TapBillsPayTest() throws Exception {
      ...
      for (int i = 0; i < BillspayCategories.size(); i++) {
         if (BillspayCategories.get(i).getText().contains("ecCash")) {
   ...
   public void errorCheckr() {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Bills Pay > No available billers category, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Bills Pay > No available EcCash billers Successful.", driver);
      }
   }
