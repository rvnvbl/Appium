...
public class Negative_TC5_Wallet_Android_CashIn_ExcessMaximum extends AndroidSetup {
	...
	public void enteringCashinValue() throws Exception {
		...
		try {
			...
			if (errorMessage.equals("Cash in amount cannot be greater than ₱40,000")) {
				System.out.println("The amount is greater than maximum");
			} else {
				markTestStatus("failed", "FAILED: The value is less than maximum.", driver);
				errorcounter++;
			}
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: The value is less than maximum.", driver);
	...
	public void validationOfAmountAndButton() {
		try {
			AndroidElement amountValidation = (AndroidElement) new WebDriverWait(driver, 30).until(
	...
	public void errorChecker() {
		...
		if (errorcounter <= 0) {
			markTestStatus("passed", "PASSED: Negative Testing > Cash in greater than maximum.", driver);
		} else {
