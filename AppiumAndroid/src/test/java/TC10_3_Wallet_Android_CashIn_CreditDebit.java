...
public class TC10_3_Wallet_Android_CashIn_CreditDebit extends AndroidSetup {
	...
	public void CashInAmountTest() throws Exception {
		...
		try {
			AndroidElement AmountTextField = (AndroidElement) new WebDriverWait(driver, 30)
	...
	public void PaymentChevronTest() throws Exception {
		try {
			...
			AndroidElement PaymentMethodChevronButton = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("wallet_cash_in_payment_arrow")));
	...
	public void CreditDebitMethodTest() throws Exception {
		...
		try {
			...
			AndroidElement PaymentMethodElement = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("wallet_cash_in_payment_info")));

			if ((PaymentMethodElement.getText()).contains("Credit/Debit Card")) {
	...
	@Test(priority = 7)
	public void CreditDebitConfirmTest() throws Exception {
		// Clicking the Confirm cash in button
		try {
			AndroidElement ConfirmCashInButton = (AndroidElement) new WebDriverWait(driver, 30)
		...
		if (errorcounter > 0) {
			markTestStatus("failed",
					"an error an occurred on Wallet > Cash In > Credit/Debit Card test, Please see logs for details",
					driver);
		} else {
			markTestStatus("passed", "Wallet > Cash In > Credit/Debit Card Module Successful", driver);
		}
	}
