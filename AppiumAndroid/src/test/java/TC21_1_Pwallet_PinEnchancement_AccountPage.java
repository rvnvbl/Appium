...
public class TC21_1_Pwallet_PinEnchancement_AccountPage extends AndroidSetup {
   ...
   private void navigateToMyAccountPage() {
      ...
      try {
         AndroidElement accountButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy.id(
               "nav_account")));
         accountButton.click();
      } catch (Exception e) {
      ...
      try {
         AndroidElement accountTittlePage = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("title")));
         try {
            Assert.assertTrue(accountTittlePage.getText().contains("Account"));
         } catch (Exception e) {
            markTestStatus("failed", "FAILED: Account page title is not same.", driver);
            errorcounter++;
         }
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Account page button is not found.", driver);
         errorcounter++;
      }

      // Look for P-Wallet Button
      try {
         AndroidElement pwalletButtton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy.id(
               "account_wallet")));
         pwalletButtton.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pwallet button is not found.", driver);
         errorcounter++;
      }
   ...
   @Test(priority = 3)
   private void validatePwalletPage() {
   ...
   @Test(priority = 4)
   private void pwalletWrongPinTest() {
   ...
   private void checkError() {
      if (errorcounter > 0) {
         markTestStatus("failed", "FAILED: an error occurred on Wallet > Account Page > Pwallet Pin test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "PASSED: Wallet > Account Page > Pwallet Pin Succesfull", driver);
      }
   }
}
