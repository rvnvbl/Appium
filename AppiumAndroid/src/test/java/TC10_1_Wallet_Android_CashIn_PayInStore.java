...
public class TC10_1_Wallet_Android_CashIn_PayInStore extends AndroidSetup {
   ...
   public void PWalletCashInTest() throws Exception {
      ...
      try {
         AndroidElement CashInButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "wallet_home_header_cash_in")));
      ...
      try {
         AndroidElement PWalletCashInTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("title")));
   ...
   public void CashInAmountTest() throws Exception {
      ...
      try {
         AndroidElement HundredPesoTile = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("wallet_cash_in_denomination_100")));
      ...
      try {
         AndroidElement AmountTextField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("wallet_cash_in_input_amount")));
   ...
   public void PaymentChevronTest() throws Exception {
      try {
         ...
         AndroidElement PaymentMethodChevronButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("wallet_cash_in_payment_arrow")));
   ...
   public void PayInStoreMethodTest() throws Exception {
      try {
         ...
         AndroidElement PayInStoreOption = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy
         ...
         PayInStoreOption.click();

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pay In Store Payment method button error / not found.", driver);
         errorcounter++;
      }
      ...
      try {
         ...
         AndroidElement PaymentMethodElement = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("wallet_cash_in_payment_info")));

         if ((PaymentMethodElement.getText()).contains("Pay In Store")) {
   ...
   @Test(priority = 8)
   public void PayInStoreConfirmTest() throws Exception {
      // Clicking the Confirm cash in button
      try {
         AndroidElement ConfirmCashInButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("wallet_cash_in_confirm")));
      ...
      // Checking if Pay in Store QR code is visible
      try {
         AndroidElement QRPayinStoreElement = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("wallet_top_up_generated_code_top_layout")));

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: QR for Pay in Store payment method is not found.", driver);
         errorcounter++;
      }
   ...
   private void errorChecker() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Wallet > Cash In > Pay In Store test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Wallet > Cash In > Pay In Store Module Successful", driver);
      }
