...
public class TC23_2_Referral_MaxReferralReach extends AndroidSetup {
   ...
   public void loginFieldsTest_Referral_ReachMax(String email, String password) throws Exception {
   ...
   private void referralPageTest() {
      ...
      try {
         ...
         if (referallTitleHeader.getText().contains("Thank you for your referrals!")) {
      ...
      try {
         ...
         if (referralDescription.getText().contains("You have reached the maximum number of referral and your effort is much appreciated.")) {
   ...
   private void errorChecker() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Referral > Max Referral, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Referral > Max Referral is passed", driver);
      }
   }
