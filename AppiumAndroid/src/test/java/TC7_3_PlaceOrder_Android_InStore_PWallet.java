...
public class TC7_3_PlaceOrder_Android_InStore_PWallet extends AndroidSetup {
   ...
   public void TapCartButtonTest() throws Exception {
      ...
      try {
         ...
         try {
            // Clicking of search field in search and searching for specific products (GLOBE CALLCARD P300)
            AndroidElement SearchField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
                  .id("toolbar_searchbar")));

            SearchField.click();

            SearchField.sendKeys("GLOBE CALLCARD P300");
         ...
         try {
            // Checking of products if all results has GLOBE CALLCARD P300 in keywords
            ...
            for (int i = 0; i < Products.size(); i++) {
               ...
               if (SpecificProductNameText.contains("GLOBE CALLCARD P300")) {
                  System.out.println("Product contains GLOBE CALLCARD P300 keyword");
         ...
         try {
            // Checking if Specific store at top is visible and correct (PPCI - NORTH COMMONWEALTH)
            AndroidElement StoreHeader = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
                  .id("toolbar_catalog_text_place")));
            String StoreHeaderString = StoreHeader.getText();
   ...
   public void PWalletMethodTest() throws Exception {
      try {
         ...
         AndroidElement PWalletOption = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy
         ...
         try {
            ...
            AndroidElement TopUpButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
                  .id("dialog_top_up_balance_option_top_up")));

            AndroidElement ChangePaymentButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
   ...
   public void PlaceOrderTest() throws Exception {
      ...
      try {
         ...
         AndroidElement PinTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "title")));
      ...
      try {
         ...
         AndroidElement PinField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "wallet_enter_pin_code")));
      ...
      try {
         ...
         AndroidElement PinSubmitButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
   ...
   public void CartCleanUpTest() throws Exception {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Place Order > In store PWallet method Module test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Place Order > In store PWallet method Module Successful", driver);
      }
   }
