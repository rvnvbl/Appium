...
public class TC26_0_CheckOut_Android_PwalletDiscount extends AndroidSetup {
   ...
   private void PwalletDiscountTest() {
      ...
      try {
         int counterError = 0;
         // AndroidElement availablePwalletBalance = (AndroidElement) new WebDriverWait(driver,
         // 30).until(ExpectedConditions.elementToBeClickable(
         // MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector()" +
         // ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
         // + "new UiSelector().text(\"Available P-Wallet Balance: \"));")));
   ...
   public void CartCleanUpTest() throws Exception {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Checkout > Pwallet Discount Modal and Display, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Checkout > Pwallet Discount Modal and Display Module Successful", driver);
      }
   }
