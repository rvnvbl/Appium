...
public class TC26_3_CheckOut_Android_Pwallet_ReflectDiscount extends AndroidSetup {
   ...
   public void PayInStoreMethodTest() throws Exception {
      ...
      swipe(driver, 500, 1800, 500, 700, 5000);
   ...
   private void PwalletDiscountTest() {
      ...
      try {
         ...
         pwalletDiscountTextField.sendKeys("1");
      } catch (Exception e) {
      ...
      try {
         AndroidElement availablePwalletBalance = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(
               MobileBy.AndroidUIAutomator("new UiScrollable(new UiSelector()" + ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
                     + "new UiSelector().text(\"Total Discount\"));")));
         System.out.println("Total Discount Label Found");
      } catch (Exception e) {
   ...
   private void retestDiscountModal() {
      ...
      try {
         AndroidElement availablePwalletBalance = (AndroidElement) new WebDriverWait(driver, 5).until(ExpectedConditions.elementToBeClickable(MobileBy
               .AndroidUIAutomator("new UiScrollable(new UiSelector()" + ".resourceId(\"com.android.settings:id/content\")).scrollIntoView("
                     + "new UiSelector().text(\"Total Discount\"));")));

         markTestStatus("failed", "FAILED: Pwallet Discount Label still foundd", driver);
         errorcounter++;
   ...
   private void errorCounterTest() {
      if (errorcounter > 0) {
         markTestStatus("failed",
               "an error an occurred on Checkout > Pwallet Discount > Display the discount on list of discount, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Checkout > Pwallet Discount > Display the discount on list of discount module Successful", driver);
      }
   }
