...
public class TC17_3_Wallet_Android_FundTransfer_Favorites extends AndroidSetup {
   String recieptEmail = "cching@yondu.com";
   String recieptWalletID = "6104529967122689";
   ...
   String ItemName = "";
   String successfulPageRecieptID = "";
   int errorcounter = 0;
   ...
   public void fillupTextboxTest() {
      try {
         ...
         emailTxt.sendKeys(recieptEmail);
      ...
      try {
         ...
         AndroidElement emailTxt = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "fund_transfer_email")));
         walletIDTxt.sendKeys(recieptWalletID);
   ...
   private void FundTransferSuccesfullTest() {
      ...
      try {
         AndroidElement transactionID = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("fund_transfer_receipt_transaction_id")));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement dateAndTime = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "fund_transfer_receipt_date_and_time")));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement recieptWalletID = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("fund_transfer_receipt_pwallet")));

         successfulPageRecieptID = recieptWalletID.getText();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement reciepEmail = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "fund_transfer_receipt_email")));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement recieptName = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "fund_transfer_receipt_name")));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement recieptAmount = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("fund_transfer_receipt_amount")));
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement fundTrasferSubmitButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("fund_transfer_receipt_button")));

         fundTrasferSubmitButton.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement addedFavoritedOkayButtonTest = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("btn_okay")));

         addedFavoritedOkayButtonTest.click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Fund Transfer Successful label is not enable/found.", driver);
         errorcounter++;
      }
   ...
   @Test(priority = 16)
   public void fundTransferModaRetestl() throws InterruptedException {
      ...
      List<AndroidElement> pwalletIDFFavorites = driver.findElements(By.id("tvPwalletId"));
      for (int i = 0; i < pwalletIDFFavorites.size(); i++) {
         System.out.println(pwalletIDFFavorites.get(i).getText());

         if (pwalletIDFFavorites.get(i).getText().equals(successfulPageRecieptID)) {
            System.out.println("Saved Account is Found on the Favorites");
         } else {
            System.out.println("PwalletID" + pwalletIDFFavorites.get(i).getText() + "Saved Pwallet ID: " + successfulPageRecieptID);
         }
      }

      List<AndroidElement> favoritesUserButton = driver.findElementsByClassName("android.view.ViewGroup");

      List<AndroidElement> filteredUser = new ArrayList<>();
      for (int i = 0; i < favoritesUserButton.size(); i++) {
         if (favoritesUserButton.get(i).getAttribute("Clickable").equals("true")) {
            filteredUser.add(favoritesUserButton.get(i));
         }
      }

      System.out.println(filteredUser.size());
      filteredUser.get(0).click();

      try {
         AndroidElement pwalletIDtextfield = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("fund_transfer_pwallet_id")));

         if (pwalletIDtextfield.getText().equals(successfulPageRecieptID)) {
            if (pwalletIDtextfield.getText().equals(pwalletIDFFavorites.get(0).getText())) {
               System.out.println("The ID is equal!");
            }
         }
         markTestStatus("passed", "PASSED: Pwallet ID textfield is found.", driver);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pwallet ID textfield is not found.", driver);
         errorcounter++;
      }
   }
   ...
   private void checkError() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Wallet > Fund Transfer > My Favorites, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Wallet > Fund Transfer > My Favorites", driver);
      }
   }
