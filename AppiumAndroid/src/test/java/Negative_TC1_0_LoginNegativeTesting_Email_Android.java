...
public class Negative_TC1_0_LoginNegativeTesting_Email_Android extends AndroidSetup {
	...
	public void NegativeLoginFieldsTestEmail(String email, String password) throws Exception {
		...
		try {
			...
			System.out.println("Test 2 passed: Incorrect Email input success");
		} catch (Exception e) {
	...
	public void LoginButtonTest() throws Exception {
		...
		try {
			...
			LoginButton.click();
			System.out.println("Test 5 passed: Login Button is shown");
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Login Button is not found.", driver);
		}

		try {
			AndroidElement ErrorPrompt = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("tvDescription")));
			
			String errorMessage = ErrorPrompt.getText();
			String result = errorMessage.replaceAll("\\s", "");
			assertTrue(result.contains("Youmayhaveenteredthewrongemailaddressorpassword."));
			
			
			System.out.println("Successful Test");
			System.out.println("Test 6 passed: Error Message prompt is shown");
			
			;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			markTestStatus("failed", "Error Prompt is not found.", driver);
			failcounter++;
		}
		
		if(failcounter>0) {
			markTestStatus("failed", "Log in > Log in As a regular User (Incorrect Email) failed", driver);
		}
		else {
			markTestStatus("passed", "Log in > Log in As a regular User (Incorrect Email) successful", driver);
		}
