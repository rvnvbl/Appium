...
public class TC11_0_DonationDrive_Android_GuestUser extends AndroidSetup {
	...
	int errorcounter = 0;
	  ...
	  @Test (priority = 0)
	  public void GuestButtonTest() throws Exception{
		// Clicking of "Continue as guest" button
	      try {
			  AndroidElement GuestButton = (AndroidElement) new WebDriverWait(driver, 30).until(
					  ExpectedConditions.elementToBeClickable(MobileBy.id("landing_guest")));
			  
	      GuestButton.click();
	   
	    // Checking if Puregold Image from homepage is visible to know if redirect is successful
  ...
  public void TapDonationDriveTest() throws Exception{
	// Scrolling to and clicking the Donation Drive button
      ...
      // Checking if Yes button for login requirement is visible to know if click is successful
	  AndroidElement YesButton = (AndroidElement) new WebDriverWait(driver, 30).until(
			  ExpectedConditions.elementToBeClickable(MobileBy.id("bPositive")));
	  }
	  catch (Exception e) {
	  ...
	  try {		  
	      // Checking and clicking of "yes" button on confirmation modal
		  AndroidElement YesButton = (AndroidElement) new WebDriverWait(driver, 30).until(
				  ExpectedConditions.elementToBeClickable(MobileBy.id("bPositive")));
	      ...
	      driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			  ...
			  markTestStatus("failed", "FAILED: Yes Button is not found.", driver);
			  errorcounter++;
		  }
  ...
  @Test (priority = 3, dataProvider = "loginData", dataProviderClass = ExcelDataSupllier.class)
  public void LoginFieldsTest_DonationDrive_GuestUser(String email, String password) throws Exception{
  ...
  public void LoginButtonTest() throws Exception{
	// Clicking of login button after the input of the above credentials
      ...
      LoginButton.click();
	    ...
	    // Checking if Puregold Image from homepage is visible to know if redirect is successful
		  AndroidElement PuregoldImageChecker = (AndroidElement) new WebDriverWait(driver, 45).until(
	              ExpectedConditions.visibilityOfElementLocated(MobileBy.id("image")));
		  
	      }
	      catch (Exception e) {
	    	  markTestStatus("failed", "FAILED: Login Error.", driver);
	    	  errorcounter++;
	      }
  ...
  @Test (priority = 5)
  public void SecondTapDonationDriveTest() throws Exception{
	// Scrolling to and clicking the Donation Drive button
	  try {
      MobileElement DonationOptionButton = (MobileElement) driver
    		  .findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()" +
	...
	// Clicking the add quantity button of the product and checking the quantity if it is now 2
	  ...
	  public void TransactionHistoryTest() throws Exception{
		...
		// Checking if "Transaction History" title is visible
		  ...
		  if (errorcounter > 0)
		  {
			  markTestStatus("failed", "an error an occurred on Donation Drive > Guest user test, Please see logs for details", driver);
		  }
		  else
		  {
			  markTestStatus("passed", "Donation Drive > Guest user Module Successful", driver);
		  }
	  }
