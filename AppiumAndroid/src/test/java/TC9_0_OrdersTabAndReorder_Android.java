...
public class TC9_0_OrdersTabAndReorder_Android extends AndroidSetup {
   ...
   public void TapOrdersButtonTest() throws Exception {
      ...
      try {
         AndroidElement OrdersButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "nav_orders")));
   ...
   public void ReorderTest() throws Exception {
      ...
      /*
       * Next tests will be for continuity / reset
       */
      try {
         // Checking of products if a product is seen in keywords
         ...
         for (int i = 0; i < Products.size(); i++) {
            ...
            if (SpecificProductNameText.contains(ItemName)) {
      ...
      try {
         // Checking if Remove button is found and clicking it after swiping left
         ...
         markTestStatus("passed", "Orders tab and Reorder module successful", driver);
      } catch (Exception e) {
   ...
   private void errorChecker() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Reorder Module test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Reorder Module Successful", driver);
      }
