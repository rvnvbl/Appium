...
public class TC5_0_Cart_Android extends AndroidSetup {
   ...
   public void CartIncreaseQuantityTest() throws Exception {
      ...
      try {
         AndroidElement IncreaseQuantityButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
   ...
   @Test(priority = 7)
   public void CartSearchIconTest() throws Exception {

      try {
         Thread.sleep(5000);

         // Clicking of search button in Cart
         ...
         markTestStatus("failed", "FAILED: Cart Search button error / not found.", driver);
         errorcounter++;
      }
   ...
   @Test(priority = 8)
   public void CartSearchKeywordTest() throws Exception {

      try {
         // Clicking of search field in search and searching for specific products
         // (Gardenia buttertoast)
         AndroidElement SearchField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "toolbar_searchbar")));

         SearchField.click();

         SearchField.sendKeys("Gardenia buttertoast");
      ...
      try {
         // Checking of products if all results has Gardenia buttertoast in keywords
         List<AndroidElement> Products = driver.findElements(By.id("cart_item_name"));
         ...
         for (int i = 0; i < Products.size(); i++) {
            ...
            String SpecificProductNameText = SpecificProductName.getText();
            if (SpecificProductNameText.contains("GARDENIA BUTTERTOAST")) {
               System.out.println("Product contains GARDENIA BUTTERTOAST keyword");
               System.out.println(i);
            } else {
               markTestStatus("failed", "FAILED: Product with no keyword from search found", driver);
               errorcounter++;
   ...
   @Test(priority = 9)
   public void CartSearchAddToCartTest() throws Exception {
      try {
         // Clicking of add to cart button on the searched item
         AndroidElement CatSearchProduct = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("search_item_add_to_cart_status_text")));

         CatSearchProduct.click();

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Product add to cart button error / not found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement MinusQuantityButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("search_product_quantity_minus")));

         AndroidElement QuantityField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("search_product_quantity")));
   ...
   public void CartReturnTest() throws Exception {
      ...
      try {
         // Checking of products if a product is seen (GARDENIA BUTTERTOAST) in keywords
         List<AndroidElement> Products = driver.findElements(By.id("cart_item_name"));
         ...
         if (counter != 0) {
            System.out.println("Successfully added searched item to cart");
         } else {
            markTestStatus("failed", "FAILED: Added product from search cannot be found.", driver);
            errorcounter++;
         }

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Cart products error / not found.", driver);
         errorcounter++;
      }
   ...
   public void CartCleanUpTest() throws Exception {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Cart Module test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Cart Module Successful", driver);
      }
   }
