...
public class TC7_0_PlaceOrder_Android_InStore_GCash extends AndroidSetup {
   ...
   public void LoginFieldsTest_Instore_GCash_Method(String email, String password) throws Exception {
   ...
   public void LoginButtonTest() throws Exception {
      ...
      try {
         ...
         /*
          * try { // Checking if an error message pops up on dashboard after logging in AndroidElement
          * CartButton = (AndroidElement) new WebDriverWait(driver, 10).until(
          ...
          * 
          * markTestStatus("failed", "FAILED: An error message popped up on dashboard after logging in.",
          * driver); } catch (Exception a) { System.out.println("No error messages popped up after login"); }
          */
   ...
   public void DesiredDateTest() throws Exception {
      try {
         ...
         // Checking all results
         outerloop: for (int i = 0; i < DateList.size(); i++) {
            ...
            // Checking all results
            for (int a = 0; i < TimeList.size(); i++) {
               AndroidElement SpecificTimeElement = TimeList.get(a);
               ...
               if (SpecificTimeAvailabilityText.equals("AVAILABLE")) {
                  ...
                  break outerloop;
                  // all loops doesn't break when the selected date does not have available time schedules
               }
            }
   ...
   public void GCashMethodTest() throws Exception {
      ...
      try {
         ...
         if ((PaymentMethodElement.getText()).equals("GCash Direct")) {
            System.out.println("Successfully changed payment method to ");
         } else {
            markTestStatus("failed", "FAILED: changed payment method to  failed", driver);
            errorcounter++;
         }
   ...
   public void CartCleanUpTest() throws Exception {
      ...
      try {
         // Checking of products if a product is seen (GLOBE CALLCARD P300) in keywords
         List<AndroidElement> Products = driver.findElements(By.id("cart_item_name"));
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Place Order > In store GCash Module test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Place Order > In store GCash Module Successful", driver);
      }
   }
