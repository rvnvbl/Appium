...
public class TC12_0_PureTreats_Android_ForMe extends AndroidSetup {
	...
	@Test(priority = 6)
	public void ForMeOptionTest() throws Exception {
		// Clicking of For Me option on "Select Recipient" modal that appeared
		try {
			AndroidElement ForMeOption = (AndroidElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("share_treats_for_me")));
	...
	public void TransactionHistoryTest() throws Exception {
		...
		if (errorcounter > 0) {
			markTestStatus("failed", "an error an occurred on Pure Treats > For Me test, Please see logs for details",
					driver);
		} else {
			markTestStatus("passed", "Pure Treats > For Me Module Successful", driver);
		}
	}
