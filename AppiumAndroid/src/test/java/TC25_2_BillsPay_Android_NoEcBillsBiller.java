...
public class TC25_2_BillsPay_Android_NoEcBillsBiller extends AndroidSetup {
   ...
   public void TapBillsPayTest() throws Exception {
      ...
      // Click ecBills
      List<AndroidElement> BillspayCategories = driver.findElements(By.id("billspay_name"));

      for (int i = 0; i < BillspayCategories.size(); i++) {
         if (BillspayCategories.get(i).getText().contains("ecBills")) {
            BillspayCategories.get(i).click();
            break;
         }
      }

      // Checking Billers Name
      List<AndroidElement> billersList = driver.findElements(By.id("bills_pay_name"));
      for (int i = 0; i < billersList.size(); i++) {
         if (billersList.get(i).getText().contains("Donation")) {
            billersList.get(i).click();
            break;
         }
      }
   ...
   private void MaintenaceBillspayCategories() throws InterruptedException {
      ...
      if (MaintenanceTitle.getText().equals("Biller Merchant Maintenance")) {
   ...
   public void errorCheckr() {
      ...
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Bills Pay > No available ECbills biller, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Bills Pay > No available Ecbills biller Successful.", driver);
      }
   }
