...
public class TC18_0_CaseOrder_Android_ProductDetails extends AndroidSetup {
   int errorcounter = 0;
   String product = "UNMEAT MEAT FREE TAPA 180G";
   String productName = "DOMEX FLOOR CLEANER ANTIBACTERIAL 1L";
   ...
   public void StoreHomepageAddToCartTest() throws Exception {
      // This will change the name of the product want to select
   ...
   @Test(priority = 5)
   private void searchAndAddTheProduct() {

      // Click the searchbar in the store homepage
      try {
         AndroidElement searchBar = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "catalog_shop_search_text")));
         searchBar.click();
      ...
      // Click the searchbar and send the "Product Name"
      try {
      ...
      // Get and the click the image of product so it will direct on the product detail page
      try {

         AndroidElement productImage = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "search_item_image")));

         List<AndroidElement> productImageList = driver.findElementsById("search_item_image");
         productImageList.get(0).click();

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Product image not found", driver);
         errorcounter++;
      }

      // Click the searchbar and send the "Product Name"
      try {
         AndroidElement productNameField = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy
               .id("item_name")));
         Assert.assertTrue(productNameField.getText().equals(productName));

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Product name field not found", driver);
         errorcounter++;
      } catch (AssertionError e) {
         System.out.println("Product name is not same");
      }

      // Click the add to cart button on the product details page
      try {
         AndroidElement addToCartButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.elementToBeClickable(MobileBy.id(
               "add_to_cart")));
         addToCartButton.click();

      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Add to cart button not found", driver);
         errorcounter++;
      }
   ...
   public void addToCaseTest() {
      ...
      try {
         ...
         markTestStatus("failed", "FAILED: Minus element not found, redirect to specific product failed.", driver);
         errorcounter++;
      }
      ...
      try {
         ...
         if (addToCartButton.isEnabled()) {
            ...
            markTestStatus("failed", "FAILED: Add to cart element not found, redirect to specific product failed.", driver);
            errorcounter++;
         }

         Thread.sleep(4000);
         List<AndroidElement> backButtonList = driver.findElementsByClassName("android.widget.ImageButton");
         backButtonList.get(0).click();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Image back element not found, redirect to specific product failed.", driver);
         errorcounter++;
      }
   }
   ...
   private void cartTest() {
      ...
      try {
         ...
         if (productCartName.equals(productName)) {
         } else {
   ...
   private void cartCaseOrderPopUp() {
      ...
      try {
         AndroidElement cartTotalQTY = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "cart_quantity")));
         String totalQRT = cartTotalQTY.getText();
         if (totalQRT.equals(totalQRT)) {
            System.out.println("The quantity is same: " + finalCartPiecePerCase + " : " + totalQRT);
         } else {
            System.out.println("The quantity is not same: " + finalCartPiecePerCase + " : " + totalQRT);
            markTestStatus("failed", "FAILED: The total is not same.", driver);
         }
   ...
   private void clearCart() {
      ...
      try {
         AndroidElement cartQuantity = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "cart_quantity")));

         cartQuantity.click();
         cartQuantity.clear();
         cartQuantity.sendKeys("1");
         driver.navigate().back();
      } catch (Exception e) {
         System.out.println("Stock confirm on cart did not appear, no need to click");
      }
   ...
   private void errorChecker() {
      if (errorcounter > 0) {
         markTestStatus("failed", "Add to cart > Product Details > Case Order, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Add to cart > Product Detail > Case Order Module Successful", driver);
      }
   }
