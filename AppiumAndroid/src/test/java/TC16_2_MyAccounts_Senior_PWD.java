...
public class TC16_2_MyAccounts_Senior_PWD extends AndroidSetup {
	...
	public void LoginFieldsTest_MyAccounts_Senior_PWD(String email, String password) throws Exception {
	...
	public void navigateToMyPWDIDTest() {
		try {
			AndroidElement myPWDbutton = (AndroidElement) driver.findElement(By.id("account_discount_id"));
			myPWDbutton.click();

			System.out.println("PWD button found");
			markTestStatus("passed", "PWD button found", driver);
		} catch (Exception e) {
			System.out.println("ERROR: PWD button not found");
			markTestStatus("failed", "PWD button not found", driver);
			errorcounter++;
		}
		...
		if (accountHeaderText.equals("My Senior Citizen/PWD ID")) {
			System.out.println("Succesfully land on My PWD Page");
			markTestStatus("passed", "Succesfully land on My PWD Page", driver);
		} else {
			System.out.println("ERROR: Not Succesfully land on My PWD Page");
			markTestStatus("failed", "Succesfully land on My PWD Page", driver);
			errorcounter++;
		}
	...
	@Test(priority = 5)
	private void viewMyIdTest() {
		AndroidElement viewMyIDButton = (AndroidElement) new WebDriverWait(driver, 30)
				.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("discount_id_view_ids_layout")));

		viewMyIDButton.click();

		AndroidElement accountHeader = (AndroidElement) new WebDriverWait(driver, 30)
				.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("discount_id_list_button")));
		String accountHeaderText = accountHeader.getText();

		if (accountHeaderText.equals("Add Senior Citizen/ PWD ID")) {
			System.out.println("Succesfully land on view id page");
			markTestStatus("passed", "Succesfully land on view id page", driver);
		} else {
			System.out.println("ERROR: Not Succesfully land on view id Page");
			markTestStatus("failed", "Succesfully land on view id Page", driver);
			errorcounter++;
		}
	...
	@Test(priority = 6)
	private void viewHistoryTest() {
		AndroidElement idHistoryButton = (AndroidElement) new WebDriverWait(driver, 30)
				.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("discount_id_history_layout")));

		idHistoryButton.click();

		AndroidElement accountHeader = (AndroidElement) new WebDriverWait(driver, 30)
				.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id("title")));
		String accountHeaderText = accountHeader.getText();

		if (accountHeaderText.equals("Discount History")) {
			System.out.println("Succesfully land on id history page");
			markTestStatus("passed", "Succesfully land on id history page", driver);
		} else {
			System.out.println("ERROR: Not Succesfully land on id history Page");
			markTestStatus("failed", "Succesfully land on id history Page", driver);
			errorcounter++;
		}
		driver.navigate().back();
	}
	...
	public void checker() {
		if (errorcounter <= 0) {
			markTestStatus("passed", "My Accounts > My PWD ID Module  is Succesful", driver);
		} else {
			markTestStatus("failed", "My Accounts > My PWD ID Module. See the logs for more details", driver);
		}
