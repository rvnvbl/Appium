...
public class TC27_Wallet_Android_PinSecurityRemoval_GeneratedQR extends AndroidSetup {
   ...
   private void PwalletTitlePageTest() {
      ...
      // Check the generate QR Code/ Barcode button
      try {
         AndroidElement genereateQRCodeButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("wallet_home_pay_via_qr_barcode")));

         genereateQRCodeButton.click();
         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Generate QR code / barcode button is not found.", driver);
         errorcounter++;
      }
   ...
   private void payViaQRBarcodePageTest() {
      // Checking the pay via qr title
      try {
         AndroidElement PwalletTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "title")));

         if (PwalletTitle.getText().equals("Pay via QR/Barcode")) {
            markTestStatus("passed", "PASSED: Succesfully land on Pay via QR/Barcode page.", driver);

         } else {
            markTestStatus("failed", "FAILED: Pay via QR/Barcode title is different.", driver);
            errorcounter++;
         }
         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Pay via QR/Barcode title is not found.", driver);
         errorcounter++;
      }

      // Checking the barcode
      try {
         AndroidElement barcode = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "payment_code_barcode")));

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Barcode is not found.", driver);
         errorcounter++;
      }

      // Checking the QR Code
      try {
         AndroidElement qrCode = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "payment_code_qr")));

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: QR code is not found.", driver);
         errorcounter++;
      }

      // Checking the "X" button
      try {
   ...
   private void confirmationPopUpTest() {
      // Checking the confirmation popup
      try {
         AndroidElement confirmationPopUpTitle = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(
               MobileBy.id("tvTitle")));

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Confirmation popup is not found.", driver);
         errorcounter++;
      }

      // Checking the confirmation pop up description
      try {
         AndroidElement confirmationPopUpDescription = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("tvDescription")));

         if (confirmationPopUpDescription.getText().equals("Are you done using these POS Payment codes?")) {
            markTestStatus("passed", "PASSED: Confirmation pop up description is correct.", driver);

         } else {
            markTestStatus("failed", "FAILED: Confirmation pop up description is incorrect.", driver);
            errorcounter++;
         }
         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Confirmation pop up description is not found.", driver);
         errorcounter++;
      }

      // Checking the "Yes" button on confirmation popup
      try {
         AndroidElement confirmationPopUpYesButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("bPositive")));

         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Confirmation popup 'YES' is not found.", driver);
         errorcounter++;
      }

      // Checking the "No" button on confirmation popup
      try {
         AndroidElement confirmationPopUpNoButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("bNegative")));
         confirmationPopUpNoButton.click();
         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Confirmation popup 'NO' is not found.", driver);
         errorcounter++;
      }
      ...
      try {
         AndroidElement confirmationPopUpYesButton = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions
               .visibilityOfElementLocated(MobileBy.id("bPositive")));
         confirmationPopUpYesButton.click();
         driver.manage().timeouts().implicitlyWait(7, TimeUnit.SECONDS);
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: Confirmation popup 'YES' is not found.", driver);
         errorcounter++;
   ...
   private void repeatProcess() {
      ...
      // Run Pwallet Title Page Test
      PwalletTitlePageTest();

      // Run the Pay Via QR Page Test
      payViaQRBarcodePageTest();

      // Run the Confirmation Popup Test
      confirmationPopUpTest();
   ...
   private void checkError() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Wallet > Pwallet > Pin Security Removal Generated QR failed", driver);
      } else {
         markTestStatus("passed", "Wallet > Pwallet > Pin Security Removal Generated QR passed", driver);
      }
   }
