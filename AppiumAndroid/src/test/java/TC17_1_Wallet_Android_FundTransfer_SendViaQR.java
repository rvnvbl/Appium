...
public class TC17_1_Wallet_Android_FundTransfer_SendViaQR extends AndroidSetup {
   ...
   public void LoginFieldsTest_FundTransfer_SendViaQR(String email, String password) throws Exception {
   ...
   public void allowQRtest() {
      try {
         ...
         markTestStatus("passed", "PASSED: Allow this time button is found.", driver);
      } catch (Exception e) {
         markTestStatus("failed", "Allow this time button is not found.", driver);
         errorcounter++;

      }

      try {
         AndroidElement QRpage = (AndroidElement) new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(MobileBy.id(
               "title")));

         String PWalletTitleText = QRpage.getText();

         if (PWalletTitleText.equals("QR Scanner")) {
            System.out.println("Successfully redirected to QR page");
         } else {
            markTestStatus("failed", "FAILED: QR page title error / not found", driver);
            errorcounter++;
         }
         driver.navigate().back();
      } catch (Exception e) {
         markTestStatus("failed", "FAILED: QR page title is not found.", driver);
         errorcounter++;
      }
   }

   @Test(priority = 10)
   public void validatePwallet() throws Exception {
      // Clicking the PWallet button
      ...
      try {
         ...
         if (PWalletTitleText.equals("P-Wallet")) {
            System.out.println("Successfully back to P-Wallet page");
         } else {
   ...
   private void checkError() {
      if (errorcounter > 0) {
         markTestStatus("failed", "an error an occurred on Wallet > Fund Transfer > Send via QR Module test, Please see logs for details", driver);
      } else {
         markTestStatus("passed", "Wallet > Fund Transfer > Send via QR succesfull", driver);
      }
   }
