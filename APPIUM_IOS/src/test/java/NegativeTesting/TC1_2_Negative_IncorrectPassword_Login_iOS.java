...
public class TC1_2_Negative_IncorrectPassword_Login_iOS extends NegativeSetupIOS {
	...
	// Negative Login Field Test
		@Test(priority = 2, dataProvider = "LoginProvider", dataProviderClass = DataProvider_Login.class)
		public void NegativeLoginPasswordTest(String email, String password) throws Exception {
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			...
			try {
				...
				LoginButton.click();
				// Assert the visibility of the error prompt message
			} catch (Exception e) {
				markTestStatus("failed", "FAILED: Login Button is not found.", driver);
				failedCounter++;
			}
			...
			try {
				Thread.sleep(5);
//				IOSElement errorPrompt = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
//						.visibilityOfElementLocated(MobileBy.name("You may have entered the wrong email address or password.")));
				IOSElement errorPrompt = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
						.visibilityOfElementLocated(MobileBy.name("Okay")));
				String errorMessage = errorPrompt.getText().trim();
				String result = errorMessage.replaceAll("\\s", "");
				System.out.println(result);
//				Assert.assertTrue(result.contains("Youmayhaveenteredthewrongemailaddressorpassword."));
				Assert.assertTrue(result.contains("Okay"));
				markTestStatus("passed", "PASSED: Incorrect password test.", driver);
			} catch (Exception e) {
				System.out.println(e.getMessage());
				markTestStatus("failed", "FAILED: Error Prompt Message is not found.", driver);
				failedCounter++;
			}
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Login Module has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Login Module successful.", driver);
		}
