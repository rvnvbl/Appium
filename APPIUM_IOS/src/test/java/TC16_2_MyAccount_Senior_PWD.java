...
public class TC16_2_MyAccount_Senior_PWD extends SetupIOS {
	...
	@Test(priority = 4)
	private void navigateToMySeniorCitizenPWDIDTest() {
		try {

			// verify if Delivery Button is visible and enabled
			IOSElement btn_SeniorPwdID = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("My Senior Citizen/PWD ID")));

			btn_SeniorPwdID.click();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			markTestStatus("passed", "PASSED: Senior/PWD ID Button Found.", driver);
		} catch (Exception ex) {
			markTestStatus("failed", "FAILED: Senior/PWD ID button is not found.", driver);
			failedCounter++;
		}
	}

	@Test(priority = 5)
	private void navigateToDiscountHistoryTest() {
		try {
			// verify if Delivery Button is visible and enabled
			IOSElement btn_DiscountHistory = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Discount History")));

			btn_DiscountHistory.click();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			markTestStatus("passed", "PASSED: Discount History Button Found.", driver);
		} catch (Exception ex) {
			markTestStatus("failed", "FAILED: Discount History button is not found.", driver);
			failedCounter++;
		}
	}

	@Test(priority = 6)
	private void navigateToViewMyCardTest() {
		driver.navigate().back();

		try {
			// verify if Delivery Button is visible and enabled
			IOSElement btn_viewMyCard = (IOSElement) new WebDriverWait(driver, 30).until(
					ExpectedConditions.visibilityOfElementLocated(MobileBy.name("View my Senior Citizen/PWD ID")));

			btn_viewMyCard.click();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
			markTestStatus("passed", "PASSED: View ID Button Found.", driver);
		} catch (Exception ex) {
			markTestStatus("failed", "FAILED: View ID button is not found.", driver);
			failedCounter++;
		}
	}
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: My Account > Discount and PWD ID.", driver);
		} else {
			markTestStatus("passed", "PASSED:  My Account > Discount and PWD ID.", driver);
		}
