 ...
 *
 */
public class TC18_1_Wallet_iOS_FundTransfer_SendviaQR extends SetupIOS {
	...
	private void sendviaPwalletID() {
		...
		try {
			IOSElement btnSendViaWalletID = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Send via QR")));
	...
	@Test(priority = 7)
	private void checkPwalletPageFromQRCamera() {
		// Click back in navigation bar
		driver.navigate().back();
		// Check if redirected to P-Wallet page
		try {
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Wallet > Fund Transfer > Send Via QR Module has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Wallet > Fund Transfer > Send via QR Module successful.", driver);
		}
