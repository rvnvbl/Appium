...
public class TC26_1_Checkout_PwalletDiscount_TextfieldTest_iOS extends SetupIOS {
...
// Clicking of login button after the input of the above credentials
	...
	private void greaterThanSubtotalThanWalletBalance() throws InterruptedException {
		// Check if pop up warning appeared to cart page
		try {
		...
		textfieldList.get(0).sendKeys("4");
		Thread.sleep(3000);
		// Proceed to Checkout Page
		try {
			// Verify if Checkout Button if visible
	...
	@Test(priority = 15, dataProvider = "GCash_method", dataProviderClass = DataProvider_Login.class)
	public void secondPaymentMethod_GCash(String Gcash) throws Exception {
	...
	private void secondPwalletDiscountButtonGcash() {
		// Verify if Special Instruction field is displayed
	...
	private void secondModalTexfieldTest() {
		...
		try {
			IOSElement itemSubtotal = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("subtotal")));
			String subtotal = itemSubtotal.getText();
			String finalSubtotal = subtotal.replace(",", "");
			dblsubtotal = Double.parseDouble(extractValue(finalSubtotal));
			System.out.println("The subtotal is: " + dblsubtotal);
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Item subtotal not found", driver);
			failedCounter++;
		...
		if (dblsubtotal > dblWalletPoints) {
			System.out.println("Subtotal is greater than wallet balance");
			Assert.assertTrue(dblTextFieldValues.equals(dblWalletPoints));
		}
...
//		}
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Checkout Module (Pwallet: Texfield Test) has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Checkout Module (Pwallet: Textfield Test) successful.", driver);
		}
