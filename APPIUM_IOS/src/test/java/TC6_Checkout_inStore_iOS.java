...
public class TC6_Checkout_inStore_iOS extends SetupIOS {
...
// Clicking of login button after the input of the above credentials
			...
			try {
			 ...
			 try {
				...
				IOSElement searchbar = (IOSElement) new WebDriverWait(driver, 30)
							.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Search for Store Branch, City or Province")));
				searchbar.click();
				driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);	
	...
	@Test (priority = 10, dataProvider = "inStore_method", dataProviderClass = DataProvider_Login.class)
	...
	public void Special_Instruction() throws Exception{
		...
		try { 
			...
			IOSElement Instruction_txtField = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("E.g. Put frozen foods in a separate bag")));
	...
	@Test (priority = 13, dataProvider = "AddVoucher", dataProviderClass = DataProvider_Login.class, enabled = false)
	public void Use_voucher(String voucherCode) throws Exception{
		...
		try {
			...
			markTestStatus("passed", "Voucher Not Available.", driver);
			driver.navigate().back();
		}
	...
	public void CheckFailedCount() throws Exception{
		...
		if(failedCounter != 0) {
			markTestStatus("failed", "FAILED: In Store Checkout Module has error/s.", driver);
		} else {
			markTestStatus("passed","PASSED: In Store Checkout Module successful.", driver);
		}
