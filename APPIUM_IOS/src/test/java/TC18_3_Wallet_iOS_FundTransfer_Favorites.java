 ...
 *
 */
public class TC18_3_Wallet_iOS_FundTransfer_Favorites extends SetupIOS {
	...
	public void fundTransferSuccessfulPageTest() {
		...
		recipientWalletID = textFieldElementClass.get(10).getText();
		recipientName = textFieldElementClass.get(14).getText();
	...
	@Test(priority = 11)
	private void verifyAccountOnFavorites() {
		// Send via Pwallet ID
		// Click Send Pwallet
		try {
			IOSElement btnSendPWallet = (IOSElement) new WebDriverWait(driver, 30)
		...
		// Check if the account is inside the favorites view all
		try {
			List<IOSElement> textFieldViewAll = driver.findElements(By.className("XCUIElementTypeStaticText"));
			int verifyCountViewAll = 0;
			for (int i = 0; i < 10; i++) {
				if (textFieldViewAll.get(i).getText().contains(recipientName)
						|| textFieldViewAll.get(i).getText().contains(recipientWalletID)) {
					verifyCountViewAll++;
					System.out.println(textFieldViewAll.get(i).getText());
				}

			}
			if (verifyCountViewAll < 2) {
				markTestStatus("failed", "FAILED: User not found on the All Favorite Page.", driver);
				failedCounter++;
			}
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Error on View All Favorites Page.", driver);
			failedCounter++;
		}

		// Back to fund transfer page
		try {
			IOSElement btnBackButton = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Fund Transfer")));
			btnBackButton.click();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		} catch (Exception e) {
		...
		try {
			// Checking the add account in the favorite list in fund transfer page
			List<IOSElement> textFieldFundTransfer = driver.findElements(By.className("XCUIElementTypeStaticText"));
			int verifyCountFundTransfer = 0;

			for (int i = 0; i < textFieldFundTransfer.size(); i++) {
				if (textFieldFundTransfer.get(i).getText().contains(recipientName)) {
					verifyCountFundTransfer++;
				}
				if (textFieldFundTransfer.get(i).getText().contains(recipientWalletID)) {
					verifyCountFundTransfer++;
				}
			}
			if (verifyCountFundTransfer < 2) {
				markTestStatus("failed", "FAILED: User not found on the Funbd Transfer Page.", driver);
				failedCounter++;
			}
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Error on Fund Transfer Page.", driver);
			failedCounter++;
		}
	}

	@Test(priority = 12)
	private void clickTheFirstFavorites() {
		List<IOSElement> favoriteListCell = driver.findElements(By.className("XCUIElementTypeCell"));
		favoriteListCell.get(0).click();

		List<IOSElement> textFieldList = driver.findElements(By.className("XCUIElementTypeTextField"));
		String PwalletID = textFieldList.get(1).getText();
		textFieldList.get(2).sendKeys("1");

		IOSElement btn_done = (IOSElement) driver.findElement(By.name("Done"));
		btn_done.click();
		driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);

		if (PwalletID.equals(recipientWalletID)) {
		} else {
			markTestStatus("failed", "FAILED: The id is not same.", driver);
			failedCounter++;
		}
	...
	@Test(priority = 13, dataProvider = "PwalletID", dataProviderClass = DataProvider_Login.class)
	private void confirmPageFavorite(String Email, String PwalletID, String amount) {
		// Getting all the text field in the confirmation page
		try {
	...
	@Test(priority = 14)
	public void secondpwalletPinPageTest() {
	...
	@Test(priority = 15)
	public void secondfundTransferSuccessfulPageTest() {
		try {
			IOSElement fundTransferSuccessfulTitle = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Fund Transfer Successful")));
		} catch (Exception e) {
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Wallet > Fund Transfer > My Favorites has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Wallet > Fund Transfer > My Favorites successful.", driver);
		}
