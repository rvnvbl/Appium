...
public class TC7_PlaceOrder_InStore_GCash extends SetupIOS {
...
//			}
	...
	public void Cart_page() throws Exception {
		...
		try {
			...
			IOSElement btn_okay = (IOSElement) new WebDriverWait(driver, 60)
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Place Order for In Store pickup using GCash Module has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Place Order for In Store pickup using GCash Module successful.", driver);
		}
