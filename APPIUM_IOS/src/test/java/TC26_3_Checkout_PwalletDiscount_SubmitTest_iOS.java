...
public class TC26_3_Checkout_PwalletDiscount_SubmitTest_iOS extends SetupIOS {
...
// Clicking of login button after the input of the above credentials
	...
	private void modalTexfieldTest() {
		...
		try {
			...
			inputTextfield.sendKeys("1.00");
			System.out.println("The textfield change to 1 peso");
	...
	private void placeOrderTest() {
		...
		try {
			...
			if (pwalletTitle.getText().contains("P-Wallet PIN")) {
				System.out.println("Pwallet page title correct");
			} else {
				System.out.println("Pwallet page title incorrect");
			}
		...
		try {
			...
			System.out.println("Pwallet Page label found");
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		} catch (Exception e) {
		...
		try
		{
			...
			for (int i = 0; i < 6; i++) {
				pwalletPinTextField.get(i).click();
		...
		try {
			...
			System.out.println("Forgot pin found");
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		} catch (Exception e) {
	...
	private void confirmationPageTest() {
		// Checking the confirmation title label
		try {
			IOSElement confirmationTitle = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Confirmation")));

			if (confirmationTitle.getText().contains("Confirmation")) {
				System.out.println("Confirmation title correct");
			} else {
				System.out.println("Confirmation title is not correct");
			}

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Confirmation title is not found", driver);
			failedCounter++;
		}

		// Checking the thank you message
		try {
			IOSElement thankyouMessage = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Thank you for shopping!")));

			try {
				Assert.assertTrue(thankyouMessage.getText().contains("Thank you for shopping!"));

			} catch (Exception e) {
				markTestStatus("failed", "FAILED: Thank you message is not correct", driver);
				failedCounter++;
			}

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Thankk you message is not found", driver);
			failedCounter++;
		}
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Checkout Module (Pwallet: Submit Test) has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Checkout Module (Pwallet: Submit Test) successful.", driver);
		}
