...
public class TC8_3_PlaceOrder_Delivery_ZeroPayment extends SetupIOS{
	...
	public void LoginFieldsTest_Delivery_ZeroPayment(String email, String password) throws Exception {
...
// Clicking of login button after the input of the above credentials
	...
	public void Verify_CartItem() throws Exception {
		...
		try {
			...
			try {
				...
				// verify if Delivery Guidelines title is visible to know if redirection is successful
			...
			try {
				// Checking if "I'm Shopping in" title is visible after clicking the address to know if redirection is successful
			...
			try {
				IOSElement searchbar = (IOSElement) new WebDriverWait(driver, 30)
						.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Search for products and brands")));
			...
			try {
				IOSElement searchbar = (IOSElement) new WebDriverWait(driver, 30)
						.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Search for product name or category")));
	...
	@Test (priority = 7)
	public void PaymentMethod_CashOnDelivery() throws Exception{
		...
		try { 
			...
			IOSElement btn_COD = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Cash On Delivery")));
			btn_COD.click();
	...
	public void Use_voucher(String voucherCode) throws Exception{
		...
		try {
			...
			IOSElement lbl_Subtotal = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("total")));
		...
		try { 
			...
			IOSElement btn_useVoucher = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Use a Voucher")));
		...
		try {
			...
			IOSElement btn_redeem = (IOSElement) new WebDriverWait(driver, 60)
		...
		try {
			...
			IOSElement btn_useNow = (IOSElement) new WebDriverWait(driver, 60)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("USE NOW")));
			btn_useNow.click();
	...
	public void CheckFailedCount() throws Exception{
		...
		if(failedCounter != 0) {
			markTestStatus("failed", "FAILED: Place Order for Delivery using Zero Payment method Module has error/s.", driver);
		} else {
			markTestStatus("passed","PASSED: Place Order for Delivery using Zero Payment method Module successful.", driver);
		}
