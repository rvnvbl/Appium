...
public class TC15_BuyLoad_RegularData_iOS extends SetupIOS {
...
//		// Check if redirected to Buy Load page
//		try {
//			IOSElement lbl_BuyLoad = (IOSElement) new WebDriverWait(driver, 30)
//					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Buy Load")));
//			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
//
//			markTestStatus("passed", "PASSED: Redirected to Buy Load Page.", driver);
//
//		} catch (Exception e) {
//			markTestStatus("failed", "FAILED: Did not redirected to Buy Load Page.", driver);
//			failedCounter++;
//		}
...
//		Input of Load amount
//		Choosing the specific load denomination
...
//		Checking of Pay Now button if enabled
		try {
			// Visibility
			IOSElement buttonRegularLoad = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("regular")));

			// Tapping the Mobile Data Tab
			buttonRegularLoad.click();
			...
			// Regular load by clicking the button
//			List<IOSElement> regularLoadAmounts = (List<IOSElement>) new WebDriverWait(driver, 30).until(
//					ExpectedConditions.visibilityOfElementLocated(MobileBy.className("XCUIElementTypeStaticText")));
			List<MobileElement> regularLoadAmounts = dataPromoContainer
					.findElementsByClassName("XCUIElementTypeStaticText");

			IOSElement chosenRegularLoadAmount = (IOSElement) regularLoadAmounts.stream()
					.filter(x -> x.getText().contains("20")).findFirst().orElse(null);
			chosenRegularLoadAmount.click();
			markTestStatus("PASSED", chosenRegularLoadAmount.getText(), driver);
			...
			System.out.println(textFieldAmount.getText()+" here     dgajsdfg");
//			boolean doesItContain = false;
			//Assert.assertTrue(textFieldAmount.getText().contains(chosenRegularLoadAmount.getText()));
			
			//markTestStatus("PASSED", textFieldAmount.getText(), driver);
			markTestStatus("PASSED", "Promo ammount has matched the displayed amount", driver);
			System.out.println(textFieldAmount.getText()+"2ndddddddd");
...
//			Checking of items to purchase if correct
			...
			Assert.assertTrue(displayedPurchasedItem.getText().trim().contains("20 TM Regular Load"));
...
//			Checking of Pay Now button if enabled
			...
			markTestStatus("passed", "PASSED: Buy Load Regular > Buy Load Regular Module successful.", driver);
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Buy Load Regular " + e.getLocalizedMessage(), driver);
		}
	}
	...
	public void CheckFailedCount() throws Exception {
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Buy Load Regular > Buy Load Regular Load Module has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Buy Load Regular > Buy Load Regular Load Module successful.", driver);
		}
	}
