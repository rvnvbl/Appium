...
public class TC10_3_Wallet_CashIn_CreditDebit_iOS extends SetupIOS {
	...
	@Test(priority = 4)
	public void CashIn_CreditDebit() throws Exception {
		...
		//Check if Credit/Debit Card button is displayed
		try {
			...
			markTestStatus("passed", "PASSED: Credit/Debit Card Button found.", driver);
...
*/
	...
	@Test (priority = 5)
	public void creditDebit_page() throws Exception{
		// Check if redirected to Credit/Debit Card page
		try {
			IOSElement lbl_creditDebit = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Order and Payment Information")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			
			markTestStatus("passed", "PASSED: Successfully Redirected to Credit/Debit Card Payment Page.", driver);
			
			Thread.sleep(5000);
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Did not redirected to Credit/Debit Card Payment Page.", driver);
			failedCounter ++;
		}
	}
	...
	public void CheckFailedCount() throws Exception{
		...
		if(failedCounter != 0) {
			markTestStatus("failed", "FAILED: Wallet > Credit/Debit Card Cash In Module has error/s.", driver);
		} else {
			markTestStatus("passed","PASSED: Wallet > Credit/Debit Card Cash In Module successful.", driver);
		}
