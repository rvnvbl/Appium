...
public class TC11_1_DonationDrive_GuestUser_iOS extends SetupIOS {

	int failedCounter = 0;

	@Test(priority = 0)
	public void ContinueAsGuest() throws Exception {
		// Clicking of Continue as Guest button on startup screen
		
		try {
			IOSElement btn_guest = (IOSElement) new WebDriverWait(driver, 60)
					.until(ExpectedConditions.elementToBeClickable(MobileBy.name("Continue as Guest")));
			btn_guest.click();
			Thread.sleep(8000);

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Continue as Guest Button is not found.", driver);
			failedCounter++;
		}
	}
...
//			driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
	...
	@Test(priority = 4)
	public void Navigate_to_DonationDrive_from_login() throws Exception {
...
//			driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		...
		try {
			IOSElement lbl_donationDrive = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Donation Drive")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	...
	public void ProceedToCheckout() throws Exception {
		...
		try {
			IOSElement lbl_contactInfo = (IOSElement) new WebDriverWait(driver, 30)
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Donation Drive as a Guest User Module has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Donation Drive as a Guest User Module successful.", driver);
		}
