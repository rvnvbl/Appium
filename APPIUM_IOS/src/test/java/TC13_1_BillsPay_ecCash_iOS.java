...
public class TC13_1_BillsPay_ecCash_iOS extends SetupIOS {
	...
	public void Navigate_to_Bills_Pay() throws Exception {
		...
		try {
			...
			btn_billsPay.click();
			markTestStatus("passed", "PASSED: Scrolled to Billspay button.", driver);
	...
	@Test(priority = 4)
	public void Navigate_to_ecCash() throws Exception {
		
		// Checking of ecCash Button if visible
		try {
			IOSElement btn_ecCash = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("ecCash")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			btn_ecCash.click();
			
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: ecCash button not found.", driver);
			failedCounter ++;
		}
		
		// Check if redirected to ecCash page
		try {
			IOSElement lbl_ecCash = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("ecCash")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			
			markTestStatus("passed", "PASSED: Redirected to ecCash Page.", driver);
			
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Did not redirected to ecCash Page.", driver);
			failedCounter ++;
		}
	...
	public void Select_Biller() throws Exception{
		...
		// Checking of Easytrip Button if visible
		try {
			IOSElement btn_easytrip = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("EASYTRIP")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			btn_easytrip.click();
			
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Easytrip button not found.", driver);
			failedCounter ++;
		}
		
		// Check if redirected to Easytrip page
		try {
			IOSElement lbl_donation = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("EASYTRIP")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			
			markTestStatus("passed", "PASSED: Redirected to Easytrip Page.", driver);
			
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Did not redirect to Easytrip Page.", driver);
			failedCounter ++;
		}
	
	}
	
	@Test(priority = 7, dataProvider = "ecCash", dataProviderClass = DataProvider_Login.class)
	public void Biller_Details(String acctNum, String mobileNum, String amount) throws Exception {
		...
		try {
			...
			//verify EASYTRIP title
			IOSElement Donation_title = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("EASYTRIP")));
		
			markTestStatus("passed", "Redirected to Biller Details page.", driver);
			
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Did not redirect to Biller Details page.", driver);
			failedCounter ++;
		}
        ...
        try {
        	...
        	List_TextField.get(0).sendKeys(acctNum);
        ...
        try {
        	...
        	List_TextField.get(1).sendKeys(mobileNum);
		...
		try { 
			...
			try { 
				...
				IOSElement lbl_checkout = (IOSElement) new WebDriverWait(driver, 30)
						.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("btn_pay_now")));
	...
	public void CheckFailedCount() throws Exception{
		...
		if(failedCounter != 0) {
			markTestStatus("failed", "FAILED: Bills Pay > ecCash Module has error/s.", driver);
		} else {
			markTestStatus("passed","PASSED: Bills Pay > ecCash Module successful.", driver);
		}
