...
public class TC19_1_CaseOrder_ProductDetails extends SetupIOS {
	String productXpath = "/XCUIElementTypeApplication/XCUIElementTypeWindow[1][4]/XCUIElementTypeButton[6]";
	int failedCounter = 0;
	int productPrice = 0;
	...
	public void LoginFieldsTest_AddToCart_CaseOrdering(String email, String password) throws Exception {
...
// Clicking of login button after the input of the above credentials
	...
	public void searchItem() throws Exception {
		...
		try {
			IOSElement searchBarProduct = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
					.visibilityOfElementLocated(MobileBy.name("Search for product name or category")));

			markTestStatus("passed", "PASSED: Successfully redirected to store homepage", driver);
			searchBarProduct.click();
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);

			IOSElement searchButton = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.iOSNsPredicateString("label CONTAINS 's'")));

			searchButton.click();
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Search button not found.", driver);
			failedCounter++;
		}
		try {
			IOSElement firstProduct = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.xpath(
							"/XCUIElementTypeApplication/XCUIElementTypeWindow[1][1]")));

			firstProduct.click();

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: First product doesnt found / error.", driver);
			failedCounter++;
		}
	...
	@Test(priority = 4)
	private void clickItemOnHomepage() {
//		try {
//
//			scrollToElement("PURE BASICS TOILET BOWL CLEANER  900ML");
//			IOSElement selectedProduct = (IOSElement) new WebDriverWait(driver, 30)
//					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.xpath(productXpath)));
...
//		}
		...
		// Look for searchbar
		try {
			IOSElement searchBarHomepage = (IOSElement) new WebDriverWait(driver, 30).until(
					ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Search for products and brands")));
			searchBarHomepage.click();
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Search bar (homepage) doesnt found / error.", driver);
			failedCounter++;
		}
		...
		try {
			IOSElement searchBar = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
					.visibilityOfElementLocated(MobileBy.name("Search for product name or category")));
			searchBar.click();
			searchBar.sendKeys("PURE BASICS TOILET BOWL CLEANER  900ML");
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Search bar (Search Page) doesnt found / error.", driver);
			failedCounter++;
		}
		...
		try {
			IOSElement searchButtonKeyboard = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
					.visibilityOfElementLocated(MobileBy.name("Search")));
			searchButtonKeyboard.click();

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Search button (Search Page) doesnt found / error.", driver);
			failedCounter++;
		}
		
		try {
			IOSElement itemSearched = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
					.visibilityOfElementLocated(MobileBy.name("detailBtn")));
			itemSearched.click();

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Item doesnt found / error.", driver);
			failedCounter++;
		}
	...
	public void addToCartQuantityTest() {
		...
		driver.navigate().back();
//		try {
//			IOSElement backButtonHomepage = (IOSElement) new WebDriverWait(driver, 30)
//					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("nav back white")));
//			markTestStatus("passed", "PASSED: Back Button Shows", driver);
//			backButtonHomepage.click();
//			driver.manage().timeouts().implicitlyWait(20, TimeUnit.SECONDS);
//		} catch (Exception e) {
//			markTestStatus("failed", "FAILED: No Back Button Shows/found.", driver);
//			failedCounter++;
//		}
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Add to Cart Module > Case Order > Product Details has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Add to Cart Module > Case Order > Product Details successful.", driver);
		}
	}
