...
public class TC25_BillsPay_BillsPayNoCategory_iOS extends SetupIOS {
	...
	public void maintenanceCheck() throws Exception {
		...
		try {
			IOSElement btn_transaction = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Bills Pay Maintenance")));
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Bills Pay > No Billspay Category Maintenance Module has error/s.",
					driver);
		} else {
			markTestStatus("passed", "PASSED: Bills Pay > No Billspay Category Maintenance Module successfull", driver);
		}
