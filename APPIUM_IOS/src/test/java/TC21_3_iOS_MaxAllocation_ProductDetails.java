...
public class TC21_3_iOS_MaxAllocation_ProductDetails extends SetupIOS {
...
// Clicking of login button after the input of the above credentials
	...
	private void navigateToProductDetailPage() throws InterruptedException {
		// Look for Sallys Deal Header
		String headerHeight = "";
		int productQTY = 998;
		int categoryHeader = 0;
		WebDriverWait wait = new WebDriverWait(driver, 30);
		try {
			IOSElement headerSallysDeal = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Sally's Deals")));
			headerHeight = getHeight(headerSallysDeal);
			System.out.println(headerHeight);
			categoryHeader = Integer.parseInt(getHeight(headerSallysDeal));
			driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
...
//		availableQTY = 50 - seconddigitSold;
		...
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		List<IOSElement> btnImgProductList = driver.findElements(By.id("detailBtn"));
		List<IOSElement> sallysDealImgProductList = new ArrayList<>();
		for (int i = 0; i < btnImgProductList.size(); i++) {
			if (Integer.valueOf(getHeight(btnImgProductList.get(i))) - Integer.valueOf(headerHeight) == 65) {
				sallysDealImgProductList.add(btnImgProductList.get(i));
			}
		}
		System.out.println("The size of Sallys Deal Image Product: " + sallysDealImgProductList.size());
		// Click add to cart button of item 1
		wait.until(ExpectedConditions.elementToBeClickable(sallysDealImgProductList.get(2))).click();

	}

	@Test(priority = 4, enabled = true)
	private void navigateToProductDetailPageEnhanced() throws InterruptedException {
		// Look for Sallys Deal Header
		String headerHeight = "";
		...
		// Click add to cart button of item 1
		wait.until(ExpectedConditions.elementToBeClickable(btnImgProductList.get(1))).click();
	...
	private void selectProduct() throws InterruptedException {
		WebDriverWait wait = new WebDriverWait(driver, 30);
		String productqty = "998";

		try {
			IOSElement addToCartButton = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.elementToBeClickable(MobileBy.name("Add to Cart")));

			addToCartButton.click();
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Add to cart button is not found.", driver);
			failedCounter++;
		}

		try {
			IOSElement qtyTextfield = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.elementToBeClickable(MobileBy.name("quantityTF")));

			qtyTextfield.clear();
			qtyTextfield.sendKeys("1001");
			driver.hideKeyboard();
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: QTY textfield is not found.", driver);
			failedCounter++;
		}
		...
		try {
			...
			markTestStatus("failed", "FAILED: Popup MaxQuantity not found / error.", driver);
			failedCounter++;
		}
		...
		try {
			...
			markTestStatus("failed", "FAILED: Max Quantity Pop Up Okay button not found / error.", driver);
			failedCounter++;
		}
		...
		try {
			...
			qtyTextfield.clear();
			qtyTextfield.sendKeys(String.valueOf(productqty));
			driver.hideKeyboard();
		} catch (Exception e) {
			markTestStatus("failed", "FAILED: QTY textfield is not found.", driver);
			failedCounter++;
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Max Allocation Passed (Product Details Page) has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Max Allocation Passed (Product Details Page).", driver);
		}
