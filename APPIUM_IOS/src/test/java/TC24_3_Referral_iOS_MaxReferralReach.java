 ...
 *
 */
public class TC24_3_Referral_iOS_MaxReferralReach extends SetupIOS {
	...
	public void LoginFieldsTest_Referral_MaxReferral(String email, String password) throws Exception {
	...
	@Test(priority = 2)
	public void navigateToMyReferralPage() throws Exception {
		// Checking of Account Button if visible
		try {
			IOSElement accountBtn = (IOSElement) new WebDriverWait(driver, 30)
		...
		// Checking of Referral Button if visible
		try {
			IOSElement referrakButton = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("Referrals")));
			referrakButton.click();

		} catch (Exception e) {
			markTestStatus("failed", "FAILED: Referral button is not found", driver);
			failedCounter++;
		}
	...
	private void validateReferralPage() {
		// Checking of Referral Button if visible
//		try {
//			IOSElement referralImg = (IOSElement) new WebDriverWait(driver, 30)
...
//		}
		...
		try {
			...
			if (!referralTitle.getText().equals("Thank you for your referrals!")) {
		...
		try {
			...
			if (!referralMessage.getText()
					.equals("You have reached the maximum number of referral and your effort is much appreciated.")) {
		...
		// Checking of Referral Button if visible
		try {
			IOSElement howLabel = (IOSElement) new WebDriverWait(driver, 30)
					.until(ExpectedConditions.visibilityOfElementLocated(MobileBy.name("howLbl")));

			if (!howLabel.getText().equals("How does referral work?")) {
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Referral > Max referral reach has error/s.", driver);
		} else {
			markTestStatus("passed", "PASSED: Referral > Max referral reach successful.", driver);
		}
