...
public class TC21_2_iOS_MaxAllocation_CartPage extends SetupIOS {
...
// Clicking of login button after the input of the above credentials
	...
	private void cartPageTest() {
		...
		try {
			...
			txtCart.get(0).clear();
			txtCart.get(0).sendKeys(productQTY);
			driver.hideKeyboard();
		} catch (Exception e) {
		...
		try {
			...
			for (; true;) {
				...
				IOSElement plusButton = (IOSElement) new WebDriverWait(driver, 30).until(ExpectedConditions
						.elementToBeClickable(MobileBy.iOSNsPredicateString("label CONTAINS 'cart plus'")));
	...
	public void CheckFailedCount() throws Exception {
		...
		if (failedCounter != 0) {
			markTestStatus("failed", "FAILED: Max Allocation Passed (Cart Page) has error/s.", driver);
		} else {
